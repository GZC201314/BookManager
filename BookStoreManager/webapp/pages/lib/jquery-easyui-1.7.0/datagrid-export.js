(function($) {
	...
	function toHtml(target, rows, footer, caption) {
		...
		var fields = dg.datagrid('getColumnFields', true).concat(dg.datagrid(
		...
		$.map(rows, function(row) {
					...
					for (var i = 0; i < fields.length; i++) {
						...
						data.push('<td style="' + tdStyle + '">' + value
								+ '</td>');
					}
					data.push('</tr>');
				});
	...
	function print(target, param) {
		...
		var content = '<!doctype html>' + '<html>' + '<head>'
				...
				+ '</head>' + '<body>' + toHtml(target, rows, footer, caption)
				+ '</body>' + '</html>';
	...
	function toExcel(target, param) {
		...
		var dg = $(target);
		var uri = 'data:application/vnd.ms-excel;base64,', template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><meta http-equiv="content-type" content="application/vnd.ms-excel; charset=UTF-8"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body>{table}</body></html>', base64 = function(
				s) {
