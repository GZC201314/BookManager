...
(function($) {
		...
		if (tt.hasClass("tree-hit")) {
			...
			if (tt.hasClass("tree-checkbox")) {
				...
				if (!tt.parent().hasClass("datagrid-cell-check")
						&& !_2a.singleSelect && e.shiftKey) {
					var _2b = $(_26).treegrid("getChildren");
					var _2c = $.easyui.indexOfArray(_2b, _2a.idField,
							_2a.lastSelectedIndex);
	...
	function _4e(_4f, _50) {
		...
		var _52 = $(_4f).datagrid("getColumnFields", true).length
				+ (_51.rownumbers ? 1 : 0);
		...
		function _54(tr, _55) {
			$("<tr class=\"treegrid-tr-tree\">"
					+ "<td style=\"border:0px\" colspan=\"" + _55 + "\">"
	...
	function _56(_57, _58, _59, _5a, _5b) {
		...
		if (_5e) {
			...
			var cc1 = _5f.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _60.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			if (!_5a) {
				_5e.children = [];
			}
	...
	function _a9(_aa, _ab) {
		...
		var _ad = _46(_aa, ref);
		_a2(_aa, {
					parent : (_ad ? _ad[_ac.idField] : null),
					data : [_ab.data]
				});
	...
	function _b5(_b6, _b7) {
		...
		_b8.total -= 1;
		$(_b6).datagrid("getPager").pagination("refresh", {
					total : _b8.total
				});
	...
	function _bb(_bc) {
		...
		function _c0(_c2) {
			...
			if (_c2.length) {
				...
				_c5.find(".tree-join").removeClass("tree-join")
	...
	$.fn.treegrid.methods = {
		...
		reloadFooter : function(jq, _d7) {
			return jq.each(function() {
						...
						if (_d8.showFooter) {
							_d8.view.renderFooter.call(_d8.view, this,
									dc.footer1, true);
							_d8.view.renderFooter.call(_d8.view, this,
									dc.footer2, false);
		...
		clearChecked : function(jq) {
			return jq.each(function() {
						...
						$.map($(_e2).treegrid("getCheckedNodes"),
								function(row) {
	...
	$.fn.treegrid.parseOptions = function(_e4) {
		return $.extend({}, $.fn.datagrid.parseOptions(_e4), $.parser
						.parseOptions(_e4, ["treeField", {
											checkbox : "boolean",
	...
	var _e5 = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_e6, _e7, _e8) {
			...
			function _ee(_ef, _f0, _f1) {
				...
				var _f3 = (_f2 ? _f2.children.length : $(_e6)
				...
				for (var i = 0; i < _f1.length; i++) {
					...
					var _f6 = _eb + "-" + (_ef ? 1 : 2) + "-"
							+ row[_e9.idField];
					_f4.push("<tr id=\"" + _f6 + "\" node-id=\""
							+ row[_e9.idField] + "\" " + cls + " " + _f5 + ">");
					_f4 = _f4.concat(_ec.renderRow.call(_ec, _e6, _ea, _ef,
							_f0, row));
					_f4.push("</tr>");
		...
		renderFooter : function(_f7, _f8, _f9) {
			...
			for (var i = 0; i < _fb.length; i++) {
				...
				_fd.push("<tr class=\"datagrid-row\" node-id=\""
						+ row[_fa.idField] + "\">");
		...
		renderRow : function(_fe, _ff, _100, _101, row) {
			...
			for (var i = 0; i < _ff.length; i++) {
				...
				if (col) {
					...
					var _104 = col.hidden ? "style=\"display:none;" + cs.s
							+ "\"" : (cs.s ? "style=\"" + cs.s + "\"" : "");
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _103 + "\" value=\""
								+ (row[_103] != undefined ? row[_103] : "")
								+ "\">");
					} else {
						var val = null;
						if (col.formatter) {
						...
						if (_103 == opts.treeField) {
							...
							if (this.hasCheckbox(_fe, row)) {
								...
								if (crow) {
									...
									$.easyui.addArrayItem(_102.checkedRows,
											opts.idField, row);
								} else {
									...
									if (prow && prow.checkState == "checked"
										...
										if (row.checked) {
											$.easyui.addArrayItem(_102.tmpIds,
													row[opts.idField]);
										}
									}
									row.checkState = flag
			...
			function _11e(rows) {
				rows.sort(function(r1, r2) {
							...
							for (var i = 0; i < _11c.length; i++) {
								var sn = _11c[i];
								var so = _11d[i];
								var col = $(_11b).treegrid("getColumnOption",
										sn);
		...
		finder : {
			getTr : function(_12b, id, type, _12c) {
				...
				if (_12c == 0) {
					...
					if (type == "body") {
						var tr = $("#" + $.data(_12b, "datagrid").rowIdPrefix
								+ "-" + _12c + "-" + id);
						if (!tr.length) {
							tr = (_12c == 1 ? dc.body1 : dc.body2)
									.find("tr[node-id=\"" + id + "\"]");
						}
						return tr;
					} else {
						if (type == "footer") {
							return (_12c == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=\"" + id + "\"]");
						} else {
							if (type == "selected") {
								return (_12c == 1 ? dc.body1 : dc.body2)
