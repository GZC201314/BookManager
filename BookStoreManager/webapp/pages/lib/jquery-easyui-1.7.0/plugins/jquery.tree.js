...
(function($) {
	...
	$.fn.tree = function(_fc, _fd) {
		...
		return this.each(function() {
					...
					if (_fe) {
						...
						_ff = $.extend({}, $.fn.tree.defaults, $.fn.tree
										.parseOptions(this), _fc);
						$.data(this, "tree", {
									options : _ff,
									tree : _1(this),
									data : [],
	...
	$.fn.tree.methods = {
		...
		update : function(jq, _117) {
			return jq.each(function() {
				_60(this, $.extend({}, _117, {
					checkState : _117.checked
							? "checked"
							: (_117.checked === false ? "unchecked" : undefined)
				}));
			});
		},
			...
			function _126(_127, _128) {
				...
				for (var i = 0; i < _128.length; i++) {
					...
					if (this.hasCheckbox(_121, item)) {
						var flag = 0;
						if (_124 && _124.checkState == "checked"
								&& opts.cascadeCheck) {
							flag = 1;
							item.checked = true;
						} else {
					...
					cc.push("<span class=\"tree-title\">"
							+ opts.formatter.call(_121, item) + "</span>");
					cc.push("</div>");
					if (item.children && item.children.length) {
