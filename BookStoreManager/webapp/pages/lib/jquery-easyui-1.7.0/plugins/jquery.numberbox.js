...
(function($) {
	...
	$.fn.numberbox.methods = {
		...
		fix : function(jq) {
			return jq.each(function() {
						...
						_13.value = null;
						var _14 = _13.parser.call(this, $(this)
										.numberbox("getText"));
	...
	$.fn.numberbox.parseOptions = function(_16) {
		var t = $(_16);
		return $.extend({}, $.fn.textbox.parseOptions(_16), $.parser
						.parseOptions(_16, ["decimalSeparator",
	...
	$.fn.numberbox.defaults = $.extend({}, $.fn.textbox.defaults, {
		...
		parser : function(s) {
			...
			if (_1d.prefix) {
				s = $.trim(s.replace(
						new RegExp("\\" + $.trim(_1d.prefix), "g"), ""));
			}
			if (_1d.suffix) {
				s = $.trim(s.replace(
						new RegExp("\\" + $.trim(_1d.suffix), "g"), ""));
			}
			if (parseFloat(s) != _1d.value) {
				if (_1d.groupSeparator) {
					s = $.trim(s.replace(new RegExp("\\" + _1d.groupSeparator,
									"g"), ""));
				}
				if (_1d.decimalSeparator) {
					s = $.trim(s.replace(new RegExp(
									"\\" + _1d.decimalSeparator, "g"), "."));
				}
				s = s.replace(/\s/g, "");
			}
