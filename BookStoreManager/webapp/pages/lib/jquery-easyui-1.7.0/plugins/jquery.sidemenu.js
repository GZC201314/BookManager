...
(function($) {
		...
		tt.tree({
					...
					onClick : function(_11) {
						if (_11.children) {
							if (_11.state == "open") {
								$(_11.target)
										.addClass("tree-node-nonleaf-collapsed");
							} else {
								$(_11.target)
										.removeClass("tree-node-nonleaf-collapsed");
							}
	...
	function _13(_14, _15, _16) {
		...
		$(_15).tooltip({
			...
			onShow : function() {
				...
				tip.add(tip.find("ul.tree")).unbind(".sidemenu").bind(
						"mouseover.sidemenu", function() {
							t.tooltip("show");
			...
			onPosition : function(_1b, top) {
				...
				if (!_17.collapsed) {
					...
					if (top + tip.outerHeight() > $(window)._outerHeight()
							+ $(document).scrollTop()) {
						top = $(window)._outerHeight()
								+ $(document).scrollTop() - tip.outerHeight();
						tip.css("top", top);
					}
				}
			}
	...
	function _2b(_2c) {
		...
		if (_2d.data) {
			...
			acc.accordion({
						fit : _2d.height == "auto" ? false : true,
						border : _2d.border,
						multiple : _2d.multiple
					});
			var _2f = _2d.data;
