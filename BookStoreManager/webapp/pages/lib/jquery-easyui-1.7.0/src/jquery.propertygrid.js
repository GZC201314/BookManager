...
(function($) {
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		...
		onAfterRender : function(target) {
			...
			opts.onResize = function(width, height) {
				view.resizeGroup(target);
				state.onResize.call($(target).datagrid('getPanel')[0], width,
						height);
			}
			view.resizeGroup(target);
		}
	...
	$.extend($.fn.datagrid.methods, {
		...
		scrollToGroup : function(jq, groupIndex) {
			return jq.each(function() {
				...
				var dc = state.dc;
				var grow = dc.body2.children('div.datagrid-group[group-index="'
						+ groupIndex + '"]');
				if (grow.length) {
					var groupHeight = grow.outerHeight();
					var headerHeight = dc.view2.children('div.datagrid-header')
							._outerHeight();
					var frozenHeight = dc.body2.outerHeight(true)
					...
					if (top < 0) {
						...
						dc.body2.scrollTop(dc.body2.scrollTop() + top
								+ groupHeight - dc.body2.height() + 18);
					}
				}
			});
		}
	...
	$.extend(groupview, {
				refreshGroupTitle : function(target, groupIndex) {
					...
					var span = dc.body1.add(dc.body2)
				...
				resizeGroup : function(target, groupIndex) {
					...
					// var ww = ht.width();
					var ww = dc.body2.children('table.datagrid-btable:first')
							.width();
					if (groupIndex == undefined) {
						var groupHeader = dc.body2
								.children('div.datagrid-group');
					} else {
						var groupHeader = dc.body2
								.children('div.datagrid-group[group-index='
										+ groupIndex + ']');
					...
					if (opts.frozenColumns && opts.frozenColumns.length) {
						...
						groupHeader.find('.datagrid-group-title').css(
								isRtl ? 'right' : 'left', -width + 'px');
					}
					if (fr.length) {
