...
public class GstoreConnector {
  ...
  private String recv() {
    try {
      ...
      // in Java String, there is no need for terminator '\0' in C. so we should omit '\0' at the
  ...
  private static byte[] packageMsgData(String _msg) {
    ...
    // in C, there should be '\0' as the terminator at the end of a char array. so we need add '\0'
    // at the end of sending message.
    data[data_len - 1] = 0;
